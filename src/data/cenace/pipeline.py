...
def update_execution_date(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not ok:
            raise RuntimeError(f"CENACE download failed for target date {target_date:%Y-%m-%d}")

        raw_dir.mkdir(parents=True, exist_ok=True)
        source_zip = tmp_raw_dir / f"{target_date:%Y%m%d}.zip"
        if source_zip.exists():
            shutil.copy2(source_zip, raw_dir / source_zip.name)
...
def main() -> None:
    ...
    n_written = update_execution_date(date.fromisoformat(args.execution_date))
